...
// LogicalAddress represents a CEC logical address (0-15) or "unknown".
//
// The CEC spec uses 0xF for both broadcast and "no/unknown" depending on the
// field; libcec internally uses -1 (cast to 0xFF) for the "no address" sentinel.
// We keep them distinct values so callers can tell them apart.
type LogicalAddress uint8

const (
	LogicalAddressTV               LogicalAddress = 0x00
	LogicalAddressRecordingDevice1 LogicalAddress = 0x01
	LogicalAddressRecordingDevice2 LogicalAddress = 0x02
	LogicalAddressTuner1           LogicalAddress = 0x03
	LogicalAddressPlaybackDevice1  LogicalAddress = 0x04
	LogicalAddressAudioSystem      LogicalAddress = 0x05
	LogicalAddressTuner2           LogicalAddress = 0x06
	LogicalAddressTuner3           LogicalAddress = 0x07
	LogicalAddressPlaybackDevice2  LogicalAddress = 0x08
	LogicalAddressRecordingDevice3 LogicalAddress = 0x09
	LogicalAddressTuner4           LogicalAddress = 0x0A
	LogicalAddressPlaybackDevice3  LogicalAddress = 0x0B
	LogicalAddressReserved1        LogicalAddress = 0x0C
	LogicalAddressReserved2        LogicalAddress = 0x0D
	LogicalAddressFreeUse          LogicalAddress = 0x0E
	LogicalAddressBroadcast        LogicalAddress = 0x0F
	LogicalAddressUnknown          LogicalAddress = 0xFF
)

// IsValid reports whether the address is a real CEC logical address (0-14).
// Broadcast (0xF) and Unknown (0xFF) are not "valid" device addresses.
func (l LogicalAddress) IsValid() bool {
	return l <= LogicalAddressFreeUse
}

// IsBroadcast reports whether this is the broadcast address (0xF).
func (l LogicalAddress) IsBroadcast() bool {
	return l == LogicalAddressBroadcast
}

// IsUnknown reports whether this is the "no address" sentinel.
func (l LogicalAddress) IsUnknown() bool {
	return l == LogicalAddressUnknown
}
...
func (l LogicalAddress) String() string {
	...
	case LogicalAddressReserved1:
		return "Reserved 1"
	case LogicalAddressReserved2:
		return "Reserved 2"
	case LogicalAddressFreeUse:
		return "Free Use"
	case LogicalAddressBroadcast:
		return "Broadcast"
	case LogicalAddressUnknown:
		return "Unknown"
	default:
		return "Unknown"
	}
...
// Parameter represents an alert parameter (libcec_parameter).
type Parameter struct {
	Type  int
	Value int64
}

// Command represents a CEC command frame
type Command struct {
...
// Adapter describes a discoverable CEC adapter (USB or built-in).
type Adapter struct {
	Path string
	Comm string
}

// Device describes a discovered CEC device on the bus.
type Device struct {
